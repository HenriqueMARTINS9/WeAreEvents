...
const FaqSection = ({ compact = false, variant = "light" }: FaqSectionProps) => {
  ...
  return (
    <section
      ...
      className={`px-6 py-20 md:py-24 ${isDark ? "bg-foreground text-primary-foreground" : "bg-background text-foreground"}`}
    >
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 xl:grid-cols-[minmax(280px,0.72fr)_minmax(0,1.28fr)] xl:gap-14 xl:px-2">
            ...
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-1">
              {[
                ...
                <div key={item.label} className={`rounded-lg p-4 ${
                  ...
                  <p className={`mt-2 font-body text-sm ${isDark ? "text-primary-foreground/55" : "text-muted-foreground"}`}>{item.text}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="space-y-3">
          {faqItems.map((item, index) => {
            ...
            return (
              <article key={item.question} className={`overflow-hidden rounded-lg ${
                isDark
                  ? "border border-primary-foreground/10 bg-primary-foreground/[0.06]"
                  : "border border-border bg-card"
              }`}>
                ...
                <div className={`grid transition-[grid-template-rows] duration-300 ease-out ${isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                  <div className="overflow-hidden">
                    <div className={`border-t px-5 py-5 md:px-6 ${isDark ? "border-primary-foreground/10" : "border-border"}`}>
                      <div className="space-y-3 pl-0 md:pl-12">
                        {item.answer.map((paragraph) => (
                          <p key={paragraph} className={`font-body text-sm leading-relaxed md:text-base ${
                            isDark ? "text-primary-foreground/68" : "text-foreground/75"
                          }`}>
                            {paragraph}
                          </p>
                        ))}
          ...
          {!compact && (
            <Link
              to="/recherche"
              className={`mt-6 inline-flex items-center gap-2 rounded-lg px-5 py-3 font-body text-sm font-semibold transition-transform hover:-translate-y-0.5 ${
                isDark
